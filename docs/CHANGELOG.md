...
- **Fixed collapsible panel thinking content duplication for DeepSeek models**: When `stream_delta_callback` is None (DeepSeek models), the guard in `_maybe_wrap_callbacks` only checked `stream_delta_callback` for the `_hls_wrapper` mark, causing `interim_assistant_callback` to be double-wrapped. Each invocation triggered `on_thinking_delta` twice, producing doubled content ("TheThe user user is is saying saying..."). Fix: Updated guard to check both `stream_delta_callback` AND `interim_assistant_callback` for the `_hls_wrapper` mark. Also improved dedup logic in `_thinking_wrapper` to use length-based dedup instead of exact text match, and handle `already_streamed` kwarg from Hermes.
- **Fixed conversation list still showing "处理中..." after completion**: The Feishu settings API may not reliably process the `summary` field when `streaming_mode: false` is in the same request. Fix: Close streaming first (without summary), then update summary as a separate API call using the new `cardkit_update_summary` method. Also handle the case where streaming was already closed (TTL timeout) by still updating the summary — this ensures the conversation list always reflects the actual card content regardless of whether streaming timed out.
