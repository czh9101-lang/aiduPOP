...
## v2.7 (2026-09-21) — Slash Confirm 原生选择 · Clarify 生命周期加固

### ✨ 新增

- **Slash Confirm 原生选择**：飞书适配器提供 `select_static + callback` 确认卡；仅提供“一次确认”与“取消”两项，不在原生卡面暴露永久授权选项。
- **确认结果锁定**：合法回调后，卡片收口为不可交互的终态；重复、过期、越权或跨会话回调均 fail-closed。确认处理的文本结果沿原会话锚点发送，并在有话题元数据时显式保持 thread reply 路由。

### 🛡️ 加固

- **Clarify 生命周期**：逐题保留独立 `clarify_id`，支持宿主标记的多选 JSON 回传，并在文字收口、超时、重置或覆盖后退役为无交互卡片。只有 `thread_id` 而没有真实消息锚点时保留宿主文字路径，避免原生卡错误落到群顶层。
- **并发与回归修复**：Clarify 回调采用一次性 claim，防双击重复 resolve；修复 submitted `CallBackCard` 的参数签名错误。
- **Clarify 分卡续写**：澄清完成后将后续流式输出切到新卡，保留前后文的自然分隔；重试不会重复切卡。
- **令牌吊销恢复**：检测到飞书令牌失效时清理 SDK 的令牌缓存并立即重试一次，避免仅因后台权限或版本变更而要求重启。

### ⚠️ 已知边界

- Hermes 当前 Slash/Clarify 宿主接口未提供可验证的原始发起人身份；本版仅能核验通用飞书授权、chat、已投递卡消息 ID、session 与请求 ID。共享会话的“仅原发起人可确认”需要宿主最小扩展后另行实现。
- Hermes 的文字确认路由仍识别手工输入的 `/always`；v2.7 仅从原生卡面隐藏该永久授权选项，未修改宿主路由。若产品要求连手工文字路径也禁止永久授权，需另获授权实施宿主最小改动。
...
- **话题并发隔离键升级 (P1)**：
  - 隔离键从单一 `chat_id` 升级为 `(thread_id or chat_id)`。飞书群聊话题内不同 topic 拥有相同 chat_id 但对应独立的 Hermes session，旧逻辑并发 seal 会把其它话题正在流式的卡片误打断封卡；升级后严格按 topic 隔离，跨话题互不干扰，同话题有序打断。
- **长任务续卡竞态泄漏保护 (P1)**：
  - 长任务流式卡被服务端超时关闭并触发重激活（续写新卡）后，旧会话超 TTL 回收时增加存活判断——目标续写会话未达到终态前延迟回收本会话，保证后续回调重定向通道畅通；同时 `_continuation_map` 增加 200 条上界防极端泄漏。
- **Agent 异常错误字符串封卡与回收 (P1)**：
  - 兼容 Hermes agent 层捕获异常后返回错误字符串的分支，COMPLETE 包装器不再因对字符串取 `.get` 抛 AttributeError，自动提取错误信息并触发终态封卡与会话回收，消除卡片永久转圈。
- **Phase 2 批处理 300315 状态再同步自愈 (P2)**：
  - 飞书偶发网络重试导致 `insert_before` 目标不存在（300315），状态自动再同步为服务端已生效真实状态，保留 dirty 标记并在本轮 flush 直接写入已存在元素，彻底根治 300315 死循环导致的答案无法上卡问题；Phase 3 补充精确点名元素丢弃与自愈重建。
- **非飞书渠道平台过滤与 /bg 锚点支持**：
  - START 钩子增加飞书平台校验，多渠道环境非飞书消息不建卡；`/bg` 后台任务流式卡显式接收并传递 `event_message_id`，卡片精准回复在用户命令消息上。
- **WS 渠道活性可观测性与门槛自检**：
  - 增加入站消息活性追踪并在 `/aowen monitor` 中展示；启动时自动校验 `lark-oapi>=1.6.4` 门槛，避免新版 Hermes 连接时发生静默重试断连。

---

## v2.4.4 (2026-09-09) — 会话中断守护 · 残留闪烁点根治版
...
- **打字机步长预设调优**：默认 `print_step` 由 4 优化为 **1**（单字符逐字敲出），消除文本分块跳跃感，恢复纯粹的机械打字机呼吸感；
- **敲字刷新率极速跃迁**：`print_frequency_ms` 默认由 70ms 升级为 **15ms** 高刷新模式，配合 `print_strategy: fast`，呈现行云流水般的流畅敲击质感；
- **纯净光脚版架构**：保持 aiduPOP 极致简约的折叠 panel 与无 footer 设计，零冗余元素，开箱即用。

---

## v2.4.2 (2026-09-05) — 宿主 Hermes Agent 拆解架构原生适配版

### 🛡️ Hardening & Compatibility — 宿主模块重构与解耦适配
- **双轨原生直通**：全面兼容 Hermes Agent September 2026 模块化解耦重构（PR #102117）；
- **契约防御**：生命周期钩子参数全面兼容，消除动态代理依赖，平稳度过 2026-09-14 弃用期；
- **全绿验证**：956 项自动化测试套件全绿通过，零语法错误。
...
- **根因复盘**：飞书整卡约 ~30KB JSON 上限。此前工具 Result/Error 输出零截断（单块可超 30K+）、N 个中等输出积少成多（最坏实测 panel 达 245KB）、answer 截断按字符计数而中文 UTF-8 占 3 字节（24K 字符可膨胀至 ~72KB），三者叠加导致偶发溢出断屏（300314 降级纯文本）。
- **第一层 · 单块钳制** `clamp_content()`（cardkit/md.py）：单块超 3000 字符自动掐中段（留头 2200 / 尾 600）；接入 tooluse 工具输出、detail 技术详情与 elements 渲染层兜底，纵深防御。
- **第二层 · panel 全局预算制**（cardkit/elements.py）：panel children 总量锁定 8000 UTF-8 字节，超支自最老内容起折叠（工具步骤保最近 2 步，推理轮次保最近 1 轮）；对上游 state 列表先拷贝再操作，杜绝污染。
- **第三层 · answer 字节级钳制** `clamp_utf8()`（cardkit/md.py）：按真实 UTF-8 字节计量（上限 13000 字节 ≈ 4300 汉字），二分查找字节安全边界；linear_mixin 全部截断点换装，根治中文三倍膨胀。
- **最坏组合实测**：245KB → 28KB，回到安全区；正常短卡片零误伤。
