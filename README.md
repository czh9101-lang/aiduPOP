...
- **Slash 确认原生化**：确认卡改用飞书原生静态选择器，仅提供“一次确认”与“取消”；处理完成后卡片收口为不可重复操作的结果。
- **Clarify 生命周期完整化**：每题独立追踪，支持多选结果，并在完成、超时、重置或覆盖后安全退役交互卡。
- **会话路由加固**：确认回调与补发文本保持原有会话/话题锚点，减少跨话题错投；无可靠消息锚点时保留宿主文字路径。

完整变更与已知边界见 [docs/CHANGELOG.md](docs/CHANGELOG.md)。
