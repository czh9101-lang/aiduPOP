...
- **卡片超限300305导致内容重复**：当 AI 调用大量工具（如44步）时，卡片元素数超过飞书200上限，封口报错300305后触发文本兜底，导致卡片和文本重复展示同一内容。修复方式：统一面板自动裁剪超出的推理轮次和工具步骤，折叠为提示行（`⚡ 还有X步早期操作已折叠`），确保卡片元素永不超限。

### Enhancement

- **封口顺序优化**：将 `_preservative_seal` 中的 `close_streaming` 移到 `batch_update` 之后执行，即先写入内容+页脚再关闭流式模式，避免封口失败时卡片被"冻住"但缺页脚的尴尬状态。
- **新增配置项**：`max_tool_steps`（默认20）和 `max_reasoning_rounds`（默认20），控制统一面板中最多显示的工具步骤数和推理轮次数，超出部分自动折叠。范围1~100。
- **卡片级元素安全网**：将元素超限安全网从面板内部（160阈值猜测）上移到卡片层。封卡时已知所有元素（面板+answer+footer+error），精确计数总tag objects，超过195（200-5缓冲）自动裁剪面板children，确保永不超200。两条封卡路径（`_preservative_seal` 逐增量封卡 + `build_unified_complete_card` 全卡重建）均覆盖。
