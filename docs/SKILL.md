...
**v1.0.3 迭代修复**：飞书 settings API 在同一请求中同时设置 `streaming_mode: false` 和 `summary` 时，`summary` 字段可能不被可靠处理。修复：采用两步操作——先关闭流式模式（不传 summary），再通过新增的 `cardkit_update_summary` 方法作为独立 API 调用更新摘要。同时处理流式模式已被 TTL 超时关闭的情况：即使 `close_streaming` 因 300309 而跳过，摘要仍通过独立调用更新，确保会话列表始终反映卡片实际内容。
