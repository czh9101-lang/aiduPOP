...
> 本表从 `docs/CHANGELOG.md` 回溯整理，配合 worklog.md 工作记录使用。
...
| **v1.2.0** | ✅ **已实施 + 真飞书验证通过**（待合并 github_sync 发布） | 待定 | v1.1.3 |
...
> 详细变更见 `docs/CHANGELOG.md` v1.2.0 条目。
...
| C2 | `build_panel_header/children` 单独入口 | ✅ 已完成 | 零 | **保守**：保留不动，补注释说明（单独入口预留未启用） |
| C3 | `TextState` 6 个死方法精简 | ⏭️ 延后到 v1.3.0 | — | v1.2.0 不做（推迟，详见 worklog Task 13） |
...
> 这些项已明确从 v1.2.0 延后，计划在 v1.3.0 处理。
...
| panel "只重建 children" 优化 | 若 flush 性能成瓶颈，真正实现 v1.1.0 预留的优化 | v1.2.0 决策 5（保守保留入口） |
...
| `docs/SKILL.md` | docs/ | 架构与配置总览 | 重大变更时 |
| `docs/AGENT_GUIDE.md` | docs/ | Agent 高密度参考 | 重大变更时 |

> v1.2.0 曾尝试为每个版本配套 PRD/DESIGN 规划文档，后因审计能力足以在版本内修复问题，不再需要独立规划文档，已删除。规划过程记录在 worklog.md。

---

*最后更新：v1.2.0。下次更新时机：v1.2.0 发布后。*
