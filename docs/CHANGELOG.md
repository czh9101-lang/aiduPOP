...
| 🐛 Bug Fix (P0) | 开 header 后 agent 报错时头部颜色与正文矛盾 | `is_error` 仅基于 `session.state` 判断，但 agent 报错时 `on_completed(error_message=...)` 只存 error_message，state 设为 COMPLETING（非 error 态）→ `is_error=False` → header 绿色"已完成"，但正文显示红色错误面板 | `is_error` 兼顾 error_message：`session.state in (CREATION_FAILED, TERMINATED) or (bool(error_message) and not is_aborted)`。报错时 header 正确用红色 |
