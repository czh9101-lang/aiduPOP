...
| # | 类型 | 问题/功能 | 原因 | 修复/说明 |
|---|------|-----------|------|-----------|
| 1 | Refactor | `monkey_patch.py` 2693行单文件过大 | 所有运行时拦截逻辑集中在一个文件，维护和定位困难 | 拆分为4个模块：`monkey_patch.py`（入口+共享状态）、`monkey_patch_gateway.py`（GatewayRunner包装器）、`monkey_patch_callbacks.py`（回调拦截）、`monkey_patch_adapter.py`（FeishuAdapter包装器） |
| 2 | Refactor | `cardkit.py` 1371行单文件过大 | 5种卡片类型的构建逻辑混在一起 | 拆分为3个模块+门面：`cardkit_elements.py`（基础元素）、`cardkit_cards.py`（卡片组装）、`cardkit_special.py`（特殊卡片）、`cardkit.py`（重导出门面） |
| 3 | Refactor | `controller_linear_mixin.py` 1410行过大，拆卡/估算逻辑与核心flush逻辑耦合 | 拆卡判断、元素估算等独立功能与核心流式逻辑混在一起 | 抽出 `linear_split.py`（拆卡/估算独立函数）、抽出 `session.py`（CardSession 数据类） |
| 4 | Docs | 根目录文档散乱 | CHANGELOG.md、SKILL.md 直接放在根目录，缺少归类 | 创建 `docs/` 文件夹，将 CHANGELOG.md、SKILL.md 移入，新增 `docs/ISSUES_TEMPLATE.md`（AI Issue 提交模板） |
| 5 | Docs | CHANGELOG.md 冗长（460行），历史版本细节过多 | v0.18.3 及更早版本的详细修复过程对当前维护价值低 | 精简为184行：最新3版保留详细表格，旧版每版1-3行总结 |
| 6 | Docs | SKILL.md 冗长（507行），版本历史与 CHANGELOG 重复 | 版本历史、Roadmap（全已完成）占大量篇幅；陷阱章节叙事过多 | 精简为269行：删除版本历史和 Roadmap，陷阱章节只保留结论和经验教训 |
| 7 | Docs | README 安装命令格式不清晰 | 3条安装命令堆在一起，分不清哪个平台 | 按 `# gitee (SSH)` / `# github (SSH)` / `# github (HTTPS)` 分组标注 |
