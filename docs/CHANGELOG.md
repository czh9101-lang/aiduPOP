...
**Bug Fixes:**
- **Deferred flush on card creation**: When reasoning/tool deltas arrive while the card is still being created (`card_message_ready=False`), `_schedule_linear_flush` now marks a `_pending_flush` flag instead of silently dropping the flush request. The card creation routine picks up the deferred flush immediately after the card is ready, ensuring content appears without waiting for the next event
- **Phase 2→3 fall-through**: Phase 2 (add panel) no longer returns immediately after adding the panel and streaming answer text. If new dirty data accumulated during the Phase 2 API calls, it falls through to Phase 3 (partial_update_element) so the panel content is updated in the same flush cycle
- **Phase 3 re-flush**: After Phase 3 completes (partial_update_element + stream_element), the flush now checks whether new dirty data arrived during the API calls and schedules a follow-up flush. This keeps the panel content updating in real-time during the thinking/tool phase, instead of batching updates until the answer completes
