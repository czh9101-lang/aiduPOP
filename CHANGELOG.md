...
| 5 | Feature | 上下文加载占位提示（Context Loading Hint） | 卡片创建后到首次内容 flush 之间有长达 43 秒空窗期（模型冷启 + 网关处理），卡片仅显示跑马灯转圈，用户以为系统卡死 | 新增 `_context_loading_element()` 占位元素：卡片创建后立即通过 `batch_update` 插入 `⏳ 正在加载上下文...` 提示（飞书 `standard_icon` token `time_outlined`，风格统一），首卡和拆卡新卡均适用；封卡时 `build_preservative_seal_actions` 自动删除占位元素；新增 i18n 条目 `loading_context` |
| 6 | Bug | 拆卡保留式封卡失败：第二张卡跑马灯不停、无 footer | `_do_linear_split()` 先创建新卡再封旧卡，创建新卡与封旧卡共享 `session.sequence`，中间并发的 `stream_element` 导致 sequence 号错乱；`_preservative_seal` 将 sequence conflict（300317）误判为幂等成功跳过，实际 close_streaming 和 batch_update 均未执行 → 跑马灯不停 + footer 缺失 | 调整 `_do_linear_split()` 操作顺序：**先封旧卡再创建新卡**（flush → seal old → create new），避免并发操作抢 sequence；新卡创建后重置 `session.sequence = 1` |
